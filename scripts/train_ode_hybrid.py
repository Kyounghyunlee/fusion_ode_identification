"""Train a multi-shot hybrid model combining the latent manifold with a shared NN source."""
...
def shot_loss(model, bundle: ShotBundle):
    t0 = bundle.ts_t[0]
    t1 = bundle.ts_t[-1]
    y0 = jnp.concatenate([
...
def loss_fn(model, bundles: Iterable[ShotBundle]):
    shot_losses = [shot_loss(model, bundle) for bundle in bundles]
...
def main():
    ...
    ap.add_argument("packs", nargs="*", help="Training pack paths (default: all data/*_torax_training*.npz)")
    ap.add_argument("--steps", type=int, default=500)
    ap.add_argument("--lr", type=float, default=1e-3)
    ap.add_argument("--out", default="outputs/hybrid_training")
    args = ap.parse_args()
    ...
    for pack_path in pack_paths:
        ...
        if rho is None:
            ...
            raise ValueError(
                "Pack %s uses a different rho grid than the previous packs" % pack_path
            )
    ...
    nn_params, nn_static = eqx.partition(nn_init, eqx.is_array)

    field = HybridField(nn=eqx.combine(nn_params, nn_static), manifold=manifold, latent=latent)
    ...
    def step(nn_params, opt_state):
        def compute_loss(p):
            nn_eval = eqx.combine(p, nn_static)
            model = eqx.tree_at(lambda m: m.nn, field, nn_eval)
            return loss_fn(model, shot_bundles)

        loss, grads = jax.value_and_grad(compute_loss)(nn_params)
    ...
    print("Starting multi-shot hybrid training...")
    for step_idx in range(args.steps):
        start = time.time()
        nn_params, opt_state, loss = step(nn_params, opt_state)
        if step_idx % 10 == 0:
            duration = time.time() - start
            print(f"Step {step_idx}: loss={loss:.4f} time={duration:.2f}s")

    trained_nn = eqx.combine(nn_params, nn_static)
    trained_model = eqx.tree_at(lambda m: m.nn, field, trained_nn)
