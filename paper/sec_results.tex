...
negative. Table~\ref{tab:comparison} reports the three configurations at
three seeds each under identical data, splits, solver, optimiser, stopping
...
Removing the residual source degrades held-out profile error from
$\SI{39}{eV}$ to $\SI{58}{eV}$ while \emph{raising} L$\to$H recall to
$0.50$. The two pathways compete to explain the same edge structure: with
the source suppressed, the only remaining way to reproduce a steepening
edge gradient is through the barrier coupling $\chi(\rho, \bact)$, so the
regime coordinate is forced to carry it. This is a direct measurement of the
identifiability concern that motivated making the source regime-blind
(\S\ref{sec:model-chi}), and it shows the concern is real: profile accuracy
and regime identifiability trade against each other in this model class.

% Remaining ablations (Delta chi = 0; no observation loss; no label loss)
% are added when their runs complete.
