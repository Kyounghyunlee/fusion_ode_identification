...
\begin{figure}[t]
  ...
  \caption{A discharge as the model sees it. Top: the D$_\alpha$ emission
  proxy with the weakly labelled H segment shaded. Middle: the causal
  inputs in fixed physical scales. Bottom: the label code, where ``--''
  marks samples excluded by the plasma gate.}
  \label{fig:data}
\end{figure}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.72\linewidth]{figures/fig_labeler}
  \caption{Anatomy of the weak labels. The raw proxy (orange) is
  contaminated by edge-localised-mode bursts; the rolling lower envelope
  (green) isolates the inter-burst baseline whose step-down is the
  transition signature. The gated variance-maximising threshold is dashed;
  the accepted H segment is shaded and the resulting transition time
  dotted.}
  \label{fig:labeler}
\end{figure}
